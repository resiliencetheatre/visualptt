...
Configure the incoming-message directory, destination for completed outgoing
recordings, and sender label in `pttkey.ini`, then run the main application:

```
visualptt
```

To use a different configuration file, pass the only supported command-line
option:

```
visualptt -i /path/to/pttkey.ini
```

The `sender_id` value is shown on transmitted video. It must contain between
one and eight characters so that it remains legible at the 160x120 video
resolution. If it is omitted, the label defaults to `EdgeCity`.
...
All `visualptt` runtime settings come from `pttkey.ini` (or the file selected
with `-i`). In particular, configure the message paths and sender label in its
`[pttkey]` section:

```ini
[pttkey]
input_dir = /path/to/incoming
output_dir = /path/to/outgoing
sender_id = EDGE
```

`input_dir` must already exist. `output_dir` is created if necessary.
...
By default, `visualptt` and `visualptt-tx` read `pttkey.ini` from their working
directory. `visualptt -i INI-FILE` selects another file for the integrated
application. To use the keyboard's AltGr key as the push-to-talk button, first
identify the keyboard's Linux input device and verify the events generated by
that key.
